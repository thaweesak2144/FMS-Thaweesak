...
export async function generateMetadata(): Promise<Metadata> {
  const [t, settings] = await Promise.all([getT(), resolveTenantSettings()]);
  const title = settings?.nameTh || settings?.nameEn || t("app.name");
  return { title, description: t("app.tagline") };
}
